...
package com.example.vincent.boxobox.services;
...
public class CardService extends HostApduService {
    ...
    public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
        ...
        byte[] result = UNKNOWN_CMD_SW;
        if (Arrays.equals(SELECT_APDU, commandApdu)) {
            String token = Connection.getToken();
            token.getBytes();
            byte[] accountBytes = token.getBytes();
            Log.i(TAG, "Sending account number: " + token);
            result = ConcatArrays(accountBytes, SELECT_OK_SW);
        }
        return result;
    }
    ...
    public static byte[] BuildSelectApdu(String aid) {
        Log.i("AID", "aid: " + aid);
        String hexString = SELECT_APDU_HEADER + String.format("%02X",
                aid.length() / 2) + aid;
        Log.i("AID", "hex total aid: " + hexString);
